"""Binary sensor platform for ATSmart (digital / contact sensors)."""
...
async def async_setup_entry(
    ...
    def _add(endpoints: list[dict]) -> None:
        new = [
            ATSmartBinarySensor(hub, entry, ep)
            for ep in endpoints
            if ep["kind"] == "contact" and ep["id"] not in known
        ]
        for ep in endpoints:
            if ep["kind"] == "contact":
                known.add(ep["id"])
...
class ATSmartBinarySensor(ATSmartEntity, BinarySensorEntity):
    _attr_device_class = BinarySensorDeviceClass.OPENING

    @property
    def is_on(self) -> bool:
        return bool(self._ep.get("detected"))
